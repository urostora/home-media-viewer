...
import { fetchDataFromApi, fetchResultFromApi } from "./helper";
...
describe('web/api/login', () => {
    ...
    it('existing user', async () => {
        ...
        const response = await fetchDataFromApi<GeneralResponseWithData<LoginResponseDataType>>(path, { email, password }, 'POST');
