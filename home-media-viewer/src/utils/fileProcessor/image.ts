...
const imageFileProcessor: FileProcessor = async (file: File, fileAlbum?: Album): Promise<boolean> => {
    ...
    if (
        typeof tags?.GPSDateStamp?.description === 'string'
        && typeof tags?.GPSTimeStamp?.description === 'string'
    ) {
    ...
    if (typeof tags?.GPSAltitudeRef?.description === 'string') { await addStringMeta(file, 'gps_altitude_ref', tags.GPSAltitudeRef.description); }
