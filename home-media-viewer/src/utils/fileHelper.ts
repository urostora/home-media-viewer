...
export const deleteMetadata = async (file: File) => {
    const metaIds = (await prisma.fileMeta.findMany({ where: { fileId: file.id }, select: { id: true}})).map(r => r.id);

    await prisma.$transaction([
        ...metaIds.map(metaId => prisma.fileMeta.delete({ where: { id: metaId }})),
        prisma.file.update({ where: { id: file.id }, data: { metadataStatus: 'New', metadataProcessedAt: null, metadataProcessingError: ''} }),
    ]);
}
